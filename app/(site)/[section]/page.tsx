...
export default async function SectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <><PageHero title={section.title} description={section.description}/><section className="section"><div className="container item-list">{items.map(item => <article className="content-item" key={item.id}><time>{item.eventDate ? formatDate(item.eventDate) : section.title}</time><div><small>{item.subtitle}</small><h2>{item.title}</h2><p>{item.summary}</p>{item.fileUrl && <a href={item.fileUrl} target="_blank" rel="noreferrer">Consultar documento →</a>}</div></article>)}{!items.length && <div className="empty"><h2>Próximamente</h2><p>Esta sección está preparada para publicar información institucional.</p></div>}</div></section></>;
}
